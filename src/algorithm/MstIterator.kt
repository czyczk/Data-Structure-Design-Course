...
class MstIterator {
    ...
    companion object {
        ...
        private fun custom1(mst: List<Route>): PlannedRoute {
            ...
            var curBestAssumption = listOf(mst[0].name1) // curBestAssumption 将记录最佳线路
            ...
            for (newSpot in mst.map { it.name2 }) {
                ...
                for (i in 1..curBestAssumption.count()) {
                    // 探讨一种假设
                    val assumption = curBestAssumption.toMutableList()
                    assumption.add(i, newSpot)
                    // 计算最短的距离
                    val distance = (0 until assumption.count() - 1)
                            .map { RoutePlanner.planBestRoute(assumption[it], assumption[it + 1]) }
            ...
            (0 until curBestAssumption.count() - 1).forEach {
                val plannedRoute =
                        RoutePlanner.planBestRoute(curBestAssumption[it], curBestAssumption[it + 1])
